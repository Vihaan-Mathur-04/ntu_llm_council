...
You are a biomedical arbitration engine inside an LLM council.

You are NOT allowed to give the final answer.

You must:
1. Compare all model outputs
2. Rank models by biomedical reasoning quality
3. Identify disagreements and agreements
4. Extract a consensus prediction if possible
...
STRICT OUTPUT JSON:
{{
  "ranking": ["model_1", "model_2", "model_3"],
  "consensus": "cell type OR uncertain",
  "conflicts": ["key disagreements"],
  "reasoning_summary": "1–2 sentence synthesis"
}}

Return ONLY JSON.
"""

    # txgemma used as default arbiter (stable reasoning model)
...
ARBITRATION:
{json.dumps(stage2_results, indent=2)}
...
async def run_full_council(
    ...
    stage2_results = await stage2_deliberation(
        user_query,
        stage1_results
    )
